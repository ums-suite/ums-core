...
The University Management System backend — a **.NET modular monolith** hosting all 16 bounded-context modules behind one deployable API, serving a multi-tenant SaaS platform for many universities (`ums-platform` [ADR-0018](https://github.com/ums-suite/ums-platform/blob/main/docs/adr/0018-multi-tenant-saas-architecture.md)) — `kart-commerce`'s counterpart, run the opposite way: one deployment, disciplined internal boundaries instead of 20 services.

## Modules

Tenancy, Identity, Organization, Admission, Academic, Student, Faculty, Finance, Hostel, Library, Alumni, Content, Documents, Notifications, Reporting, Audit — see [`ums-platform`'s module boundary map](https://github.com/ums-suite/ums-platform/blob/main/docs/architecture/module-boundaries.md) for the dependency graph and [`docs/services/`](https://github.com/ums-suite/ums-platform/tree/main/docs/services) for each module's full requirement spec.
