...
public static class TransactionalAuditWriter
{
    ...
    /// <summary>A commit with NO audit write - reserved for the embargo-lift sweep worker's own no-op ticks (nothing lapsed this pass) where nothing was actually mutated. Every REAL mutation (including the worker's own successful lift) still goes through <see cref="CommitWithAuditAsync"/>.</summary>
