...
/// <summary>Reporting's own schema (ADR-0004: <c>reporting</c>) - mirrors every other module's own DbContext shape exactly, including the outbox-drain-on-SaveChanges mechanism and the Postgres-exception translation pattern.</summary>
    ...
    : DbContext(options), IUnitOfWork, IDomainEventRecorder
{
    ...
    internal DbSet<DashboardMetric> DashboardMetrics => Set<DashboardMetric>();

    internal DbSet<RegulatoryReportDefinition> RegulatoryReportDefinitions => Set<RegulatoryReportDefinition>();

    internal DbSet<RegulatoryReportRun> RegulatoryReportRuns => Set<RegulatoryReportRun>();
    ...
    private static bool TryTranslateUniqueViolation(DbUpdateException exception, out DuplicateValueException translated)
    {
        if (exception.InnerException is PostgresException { SqlState: PostgresErrorCodes.UniqueViolation } postgresException)
        {
            ...
            {
                case "ux_regulatory_report_definitions_name":
                    translated = new DuplicateValueException("RegulatoryReportDefinition", "Name", "unknown");
                    return true;
            }
        }
