...
/// <summary>Boots the real <c>UMS.Host</c> pipeline against real, disposable Postgres/Redis containers - mirrors <c>StudentApiFixture</c>/<c>FacultyApiFixture</c> exactly.</summary>
public sealed class AcademicApiFixture : WebApplicationFactory<Program>, IAsyncLifetime
{
    private readonly PostgreSqlContainer _postgres = new PostgreSqlBuilder("postgres:17-alpine")
        .WithDatabase("ums_academic_it")
        .WithUsername("ums")
    ...
    /// <summary>Exposes the real Postgres connection string so tests can verify rows Academic's own repositories don't expose (e.g. Audit's <c>audit_log_entries</c>, Faculty's own outbox-projected <c>course_assignments</c>) - mirrors Student/Faculty's own remarks.</summary>
